...
class Ced_CsMarketplace_Block_Extensions extends Mage_Adminhtml_Block_System_Config_Form_Fieldset
{
    ...
    public function render(Varien_Data_Form_Element_Abstract $element)
    {
		...
		$url = Mage::helper('adminhtml')->getUrl('adminhtml/adminhtml_license/checkupdate',array('_secure'=>$secure));
		$html .= '<button class="primary" onclick="setLocation(\''.$url.'\')" style="float: right;margin-right: 246px;" type="button" title="Check For Updates"><span>Check For Updates</span></button>';
	...
	protected function _getFieldHtml($fieldset, $moduleName,$currentVersion = '0.0.1',$moduleProductName = '')
    {
		...
		$warning = '';
		$releaseVersionHtml = ' (Verison Up to date)';
		if ($allExtensions && isset($allExtensions[$moduleName])) {
			...
			if(strlen($warning) == 0) {
				...
				$releaseVersionHtml = '<a href="'.$url.'" target="_blank" title="Upgarde Available('.$releaseVersion.')">'.' (Available Verison '.$name.'-'.$releaseVersion.')</a>';
			//	$releaseVersion = '<a href="'.$url.'" target="_blank" title="Upgarde Available('.$releaseVersion.')">'.$name.'-'.$currentVersion.'</a>';
			} else {
				$releaseVersionHtml = '<div class="notification-global"><strong class="label">'.$warning.'</strong></div>';
			}
		}
		...
		if(strlen($data) == 0) {
			$title = Mage::helper('csmarketplace')->__('Enter the valid license after that you have to click on Save Config button.');
		...
		$field = $fieldset->addField(strtolower($moduleName), $type,//this is the type of the element (can be text, textarea, select, multiselect, ...)
			array(
				...
				'after_element_html' => $buttonHtml.$releaseVersionHtml,
			))->setRenderer($this->_getFieldRenderer());
