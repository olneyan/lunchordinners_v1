 ...
 * CedCommerce
  ...
  */
class Ced_CsVAttribute_Model_Sales_Order_Pdf_Invoice extends Mage_Sales_Model_Order_Pdf_Invoice
{
	 /**
     * Return PDF document
     *
    ...
    public function getPdf($invoices = array())
    {
        ...
        foreach ($invoices as $invoice) {
            ...
            $page  = $this->newPage();
            $vorder=Mage::getModel('csorder/vorders')->getVorderByInvoice($invoice);
			$this->setVorder($vorder);
			...
			$vendorIdProductId = Mage::getModel('csmarketplace/vproducts')
									->getVendorIdByProduct($item->getProductId());
                if ($item->getOrderItem()->getParentItem() || $vendorId!=$vendorIdProductId
				) {
                    continue;
                }
                /* Draw item */
    ...
    protected function insertOrder(&$page, $obj, $putOrderId = true)
    {
		...
		$var=Mage::getModel('csmarketplace/vendor')->load($vendor_id);
		$shopurl=$var['shop_url'];
        $seller=Mage::getModel('csvattribute/vendor_attribute')->getCollection();
