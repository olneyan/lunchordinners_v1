...
class Ced_CustomApi_Model_Product_Api extends Ced_VendorApi_Model_Product_Api
{
	...
	public function getListProduct($collection,$sort,$customerid=null) 
	{
		...
		{
			$proArray[$val->getData('category_id')][]= $val;
		}
		...
		foreach ($proArray as $k=>$v)
		{
		   ...
		   $categ_product =[];$count = 0;
		foreach ( $proArray[$k] as $product ) {
			
			$productId = $product->getId ();
			$pDesc = $product->getShortDescription();
		...
		$finalArray[]['category']=['id'=>$k,'name'=>$_category->getName(),'description'=>$catDesc,'products'=>$categ_product];
		
		}
		//print_r($finalArray);die("gfkj");
		return $finalArray;
	}
	...
	public function items($data,$filters = null, $store = null)
	{
		...
		if(count($collection)>0){
			...
			foreach ($attributes as $attribute) {
				...
				if($attribute->getData('frontend_input') == 'time')
				{
					...
					if($day ==0)
					{
						if (strpos($attribute->getData('attribute_code'), 'sun') !== false) {
							...
							$value= $vendor->getData($attribute->getData('attribute_code'));
                            $value = explode(',',$value);
                            $value = implode(':',$value);
                   ...
                   if (strpos($attribute->getData('attribute_code'), 'open_time') !== false) {
                    	$vendorProfile['open_time']= $value;
                   }	
                   else if (strpos($attribute->getData('attribute_code'), 'close_time') !== false) {
                   	    $vendorProfile['close_time']= $value;
                   }
                  
				}
				elseif($attribute->getData('attribute_code') == "delivery_fee" || $attribute->getData('attribute_code') == "min_freeship")
				{
					$currency = Mage::app()->getLocale()->currency(Mage::app()->getStore()->getCurrentCurrencyCode())->getSymbol();
				
					if($vendor->getData($attribute->getData('attribute_code')))
					{
						$value = $currency.$vendor->getData($attribute->getData('attribute_code'));
						$vendorProfile[$attribute->getData('attribute_code')]= $value;
					}
				}
				else if($attribute->getData('attribute_code') == "food_group")
				{
					$values = explode(',',$vendor->getData($attribute->getData('attribute_code')));
					...
					$vendorProfile[$attribute->getData('attribute_code')]= $result;
					
				}
				else{
					$vendorProfile[$attribute->getData('attribute_code')]= $vendor->getData($attribute->getData('attribute_code'));
				}
			}
			if(!isset($vendorProfile['min_freeship']))
			{
				$vendorProfile['min_freeship'] = $currency.'0';
			}
			...
			if(isset($vendorProfile['company_banner']))
				$vendorProfile['company_banner'] = ($vendor->getData('company_banner')=='')?Mage::getBaseUrl():Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA).$vendorProfile['company_banner'];
			if(isset($vendorProfile['company_logo']))
				$vendorProfile['company_logo'] = ($vendor->getData('company_logo')=='')?Mage::getBaseUrl():Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA).$vendorProfile['company_logo'];
