 ...
 * CedCommerce
  ...
  */
class Ced_MobiconnectAdvCart_Block_Adminhtml_Widget_Grid_Serializer extends Mage_Core_Block_Template {
	/**
	 ...
	 *
	 * @param
	 *        	Mage_Adminhtml_Block_Widget_Grid | string $grid grid object or grid block name
	 * @param string $callback
	 *        	block method to retrieve data to serialize
	 * @param string $hiddenInputName
	 *        	hidden input name where serialized data will be store
	 * @param string $reloadParamName
	 *        	name of request parametr that will be used to save setted data while reload grid
	 */
	public function initSerializerBlock($grid, $callback, $hiddenInputName, $reloadParamName = 'entityCollection') {
		...
		if ($grid instanceof Mage_Adminhtml_Block_Widget_Grid) {
			$this->setGridBlock ( $grid )->setInputElementName ( $hiddenInputName )->setReloadParamName ( $reloadParamName )->setSerializeData ( $grid->$callback () );
		}
	}
}
?>