...
class Ced_VendorApi_Model_Vendor_Api extends Mage_Api_Model_Resource_Abstract
{
	...
	{	
		...
		//	var_dump(json_encode($customerData['vendor']));die;
			...
			//Mage::log($vendor->getStatus(),null,'venreg.log');
			//Mage::log(Ced_CsMarketplace_Model_Vendor::VENDOR_APPROVED_STATUS,null,'venreg.log');
	...
	public function info($vendorId, $attributes = null)
	{
		...
		$vendor['profile_picture']= Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA).$vendor['profile_picture'];
		$vendor['company_logo']= Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA).$vendor['company_logo'];
		$vendor['company_banner']= Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA).$vendor['company_banner'];
		$data = array (
					'data' => array (
											'info'=> $vendor,
											'success' => true ,
	...
	public function items($data)
	{
		...
		foreach ($collection as $vendor) { 
			...
			$url=$vendors[$count]['company_logo'];
			$newurl= $helper->init($url)->__toString ();
		...
		if(count($vendors)){
			...
			$response['data']['banner_url']=$helper->init($img,'csbanner')->__toString ();
